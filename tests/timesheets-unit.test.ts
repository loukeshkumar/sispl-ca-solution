...
const entry = (overrides: Partial<TimeEntryRow>): TimeEntryRow => ({
  ...
  clientName: "Aurora Textiles", workLabel: null, taskTitle: null, billable: true, narration: "Work",
  ...overrides,
});
...
test("engagement effort groups by client, splits billable, and ignores internal time", () => {
  const engagements = buildEngagementEffort([
    ...
    entry({ id: "3", minutes: 200, billable: true, clientName: "Koshi Infra" }),
    entry({ id: "4", minutes: 60, billable: false, clientName: null }),
  ]);
